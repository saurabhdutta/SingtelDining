...
- (void) showAR:(NSMutableArray *) listings owner:(id) o callback:(SEL) cb{
  ...
  if ([listings count] > 0) {
    ...
    NSLog(@"%d items will shown on AR View", [listings count]);
    for (int i=0; i< [listings count]; i++){
      
      ListObject* obj = [listings objectAtIndex:i];
      
      //NSLog(@"%d. add item:%@ onto AR View", i, obj.title);
      CGFloat lat = [obj.latitude floatValue];
      CGFloat lng = [obj.longitude floatValue];
      
      tempLocation = [[CLLocation alloc] initWithLatitude:lat longitude:lng];
      tempCoordinate = [ARGeoCoordinate coordinateWithLocation:tempLocation locationTitle:obj.title];
      tempCoordinate.index = [obj.uid intValue];
      tempCoordinate.subtitle = [NSString stringWithFormat:@"%0.1f km", obj.distance];
      tempCoordinate.subtitle2 = obj.address;
      
      CoordinateView* cv = [[CoordinateView alloc] initForCoordinate:tempCoordinate owner:o callback:cb];
      [arView addCoordinate:tempCoordinate augmentedView:cv animated:NO];
